...
export async function venueRoutes(app: FastifyInstance) {
  ...
  app.get('/:id/events', { preHandler: [(app as any).authenticate] }, async (request) => {
    ...
    const total = (await query('SELECT COUNT(*) as c FROM events WHERE venue_id = $1', [id])).rows[0].c;
    const events = (await query(
      'SELECT * FROM events WHERE venue_id = $1 ORDER BY starts_at DESC LIMIT $2 OFFSET $3',
      [id, lmt, offset]
    )).rows;
