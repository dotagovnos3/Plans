...
export async function groupRoutes(app: FastifyInstance) {
  ...
  app.post('/', { preHandler: [(app as any).authenticate] }, async (request, reply) => {
    ...
    const group = (await query(
      'INSERT INTO groups (creator_id, name) VALUES ($1, $2) RETURNING *',
      [userId, name]
    )).rows[0];

    await query("INSERT INTO group_members (group_id, user_id, role) VALUES ($1, $2, 'member')", [group.id, userId]);
    for (const mid of (member_ids || [])) {
      if (mid !== userId) {
        await query("INSERT INTO group_members (group_id, user_id, role) VALUES ($1, $2, 'member') ON CONFLICT DO NOTHING", [group.id, mid]);
        await query(
          `INSERT INTO invitations (type, target_id, inviter_id, invitee_id, status) VALUES ('group', $1, $2, $3, 'pending')`,
          [group.id, userId, mid]
        );
        await query(
          `INSERT INTO notifications (user_id, type, payload) VALUES ($1, 'group_invite', $2)`,
          [mid, JSON.stringify({ group_id: group.id, inviter_name: (await query('SELECT name FROM users WHERE id = $1', [userId])).rows[0]?.name })]
        );
      }
    }
  ...
  app.post('/:id/members', { preHandler: [(app as any).authenticate] }, async (request, reply) => {
    ...
    await query("INSERT INTO group_members (group_id, user_id, role) VALUES ($1, $2, 'member') ON CONFLICT DO NOTHING", [id, user_id]);
    await query(
      `INSERT INTO invitations (type, target_id, inviter_id, invitee_id, status) VALUES ('group', $1, $2, $3, 'pending')`,
      [id, userId, user_id]
    );
    await query(
      `INSERT INTO notifications (user_id, type, payload) VALUES ($1, 'group_invite', $2)`,
      [user_id, JSON.stringify({ group_id: id, inviter_name: (await query('SELECT name FROM users WHERE id = $1', [userId])).rows[0]?.name })]
    );
